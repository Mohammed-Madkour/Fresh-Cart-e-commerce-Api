...
const getAllCartItems = catchError(async (req, res) => {
  ...
  let data = await cartModel.find({ userId }).populate({
    ...
    select: "subCategoryId brandId categoryId images title price",
  });
  res.status(201).json({ statsMessage: "success", data });
});
...
const deleteSpecificProduct = catchError(async (req, res) => {
  ...
  let data = await cartModel.findByIdAndDelete(productId).select("id");
  if (!data) {
    return res.status(400).json({ message: "couldn't delete product" });
  }
  res.status(201).json({
    status: "success",
...
export { addCart, getAllCartItems, updateCartQuantity, deleteSpecificProduct ,deleteCart};
